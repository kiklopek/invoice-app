-- GPC bank-statement account numbers are decoded by readAccount()
-- (src/lib/gpc-parser.ts) against a Czech weighted mod-11 check digit. When
-- BOTH candidate readings (KM-permuted and raw) fail that check, the parser
-- previously returned its best guess anyway with nothing distinguishing it
-- from a verified-good account number -- the guess could feed the "account"
-- auto-match tier and get learned into counterparty_payment_accounts for
-- future automatic matches. The TS side now flags this
-- (counterparty_account_verified: false); this migration threads that flag
-- through the database so the booking transaction -- the one that actually
-- protects the money -- never trusts an unverified account number, while
-- still allowing the payment itself to be accepted and matched by other
-- evidence (VS, invoice-number reference, payer name).

alter table public.bank_statement_entries
  add column counterparty_account_verified boolean not null default true;

comment on column public.bank_statement_entries.counterparty_account_verified is
  'False when the GPC parser could not verify this account number against the Czech mod-11 checksum on either candidate reading. Must never be trusted for the "account" auto-match tier or for learning into counterparty_payment_accounts.';

-- Reissue create_bank_statement_preview_v1 (originally defined as
-- create_bank_statement_preview in 20260915095449_add_bank_statement_imports.sql,
-- renamed in 20260918111705_robust_reconciliation.sql, never redefined
-- since) to carry the new column from entry_rows into bank_statement_entries.
create or replace function public.create_bank_statement_preview_v1(
...
begin
  ...
  insert into public.bank_statement_entries(
    ...
    booked_on, amount, currency, variable_symbol, counterparty_name, counterparty_account,
    counterparty_account_verified, note,
  ...
  select target_org, result_import.id, row.line_number, row.record_type, row.fingerprint,
    ...
    row.counterparty_name, row.counterparty_account, coalesce(row.counterparty_account_verified,true), row.note,
    row.proposal_kind, row.proposal_confidence,
  ...
  from jsonb_to_recordset(entry_rows) as row(
    ...
    counterparty_account text, counterparty_account_verified boolean, note text, proposal_kind text, proposal_confidence text,
...
-- Reissue reconcile_bank_statement (current body: 20260918220000_single_engine_version.sql)
-- with the account-evidence re-check and the learning insert both gated on
-- a verified account number. Every other line is unchanged.
