-- Name + exact amount books automatically.
--
-- Enabled at the owner's explicit instruction, after the tradeoff was put to
-- them: a payer name is not a reference to an invoice. It establishes that this
-- customer sent exactly this much, never which invoice they meant. It lands on
-- the right one whenever that amount belongs to only one of their open invoices,
-- and on the wrong one when the payment was really for an invoice not yet in the
-- ledger. Nothing here can detect that difference, so every booking this rule
-- makes records on the payment why it was made.
--
-- What still holds it back: the payment may not predate the invoice, the amount
-- must be the exact remaining balance, and the caller's uniqueness pass must
-- have found exactly one candidate in both directions.

-- Name comparison, mirroring the web tier's: strip diacritics, drop everything
-- that is not a letter or digit, and compare the shorter name as a prefix of the
-- longer -- banks truncate the payer field ("ESTIMATIC SYSTEMS S." for
-- "ESTIMATIC Systems s.r.o."), so equality would never fire. The 8-character
-- floor is what keeps a surname plus an initial from agreeing with half a ledger.
create or replace function private.names_match(payer text, counterparty text)
returns boolean language sql immutable as $$
  with normalized as (
    select regexp_replace(upper(translate(coalesce(payer,''),'áàâãäåčćçďéèêëěíìîïĺľňñóòôõöŕřšśťúùûůüýÿžźżÁÀÂÃÄÅČĆÇĎÉÈÊËĚÍÌÎÏĹĽŇÑÓÒÔÕÖŔŘŠŚŤÚÙÛŮÜÝŸŽŹŻ','aaaaaacccdeeeeeiiiillnnooooorrsstuuuuuyyzzzAAAAAACCCDEEEEEIIIILLNNOOOOORRSSTUUUUUYYZZZ')),'[^A-Z0-9]','','g') as a,
           regexp_replace(upper(translate(coalesce(counterparty,''),'áàâãäåčćçďéèêëěíìîïĺľňñóòôõöŕřšśťúùûůüýÿžźżÁÀÂÃÄÅČĆÇĎÉÈÊËĚÍÌÎÏĹĽŇÑÓÒÔÕÖŔŘŠŚŤÚÙÛŮÜÝŸŽŹŻ','aaaaaacccdeeeeeiiiillnnooooorrsstuuuuuyyzzzAAAAAACCCDEEEEEIIIILLNNOOOOORRSSTUUUUUYYZZZ')),'[^A-Z0-9]','','g') as b
  )
  select least(length(a),length(b))>=8
     and left(a,least(length(a),length(b)))=left(b,least(length(a),length(b)))
  from normalized
$$;

comment on function private.names_match(text,text) is
  'Whether a bank-truncated payer name agrees with a counterparty name. Evidence for ranking and, by the owner''s choice, for unattended booking when combined with an exact amount.';

-- Why an automatic booking happened, kept on the payment so it can be reviewed
-- later without reconstructing the matcher's reasoning from the statement.
alter table public.bank_payments add column if not exists match_reason text;
...
-- Worker selection and preview stamping follow the engine version.
  ...
  for s in select * from public.bank_statement_imports i where status='review'
    and (commit_requested or (automation_mode='automatic'
      ...
      select 1 from public.bank_statement_entries e where e.import_id=i.id and e.proposal_confidence='safe'
        and e.matching_version='v4' and e.bank_payment_id is null and e.disposition='accepted')))
...
begin
  ...
  if not (result->>'duplicate')::boolean then
    update public.bank_statement_entries e set matching_version='v4' where e.import_id=result_import_id;
