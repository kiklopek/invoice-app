-- The engine version gets ONE home.
--
-- It was written out by hand in three places -- the reconciler's entry filter,
-- the worker's selection, and the preview's stamp -- and the bump to v5 reached
-- only two of them. The reconciler went on demanding v4 while every new preview
-- was stamped v5, so the unattended pass selected no entries at all and
-- reported a clean run: zero imported, zero errors, nothing to see. A silent
-- no-op is the worst possible failure here, because it looks like success.
--
-- All three now read this function, so a bump is one line and cannot desync.

create or replace function private.matching_engine_version() returns text
language sql immutable as $$ select 'v5' $$;

comment on function private.matching_engine_version() is
  'The matching rules version. Entries are stamped with it at preview time and only entries carrying it may be booked unattended -- so proposals prepared by older, differently-reasoning rules are never committed under newer ones.';
  ...
  for s in select * from public.bank_statement_imports i where status='review'
    and (commit_requested or (automation_mode='automatic' and exists(
      select 1 from public.bank_statement_entries e where e.import_id=i.id and e.proposal_confidence='safe'
        and e.matching_version=private.matching_engine_version() and e.bank_payment_id is null and e.disposition='accepted')))
...
-- Stamp previews from the same single source.
...
begin
  ...
  if not (result->>'duplicate')::boolean then
    update public.bank_statement_entries e set matching_version=private.matching_engine_version() where e.import_id=result_import_id;
