...
--
-- schema.sql had been kept in sync with the cumulative effect of the
-- individually tracked migrations below (visible from its own internal
-- "Consolidated from: ...sql" comments), which made it structurally
-- incompatible with being replayed *before* those same migrations on a
-- fresh database (duplicate policy names, functions referenced under
-- schema names they did not have yet at that point in history, etc).
-- This migration replaces all of them as a single idempotent baseline:
-- every statement is safe to run both against an empty database and
-- against the already-deployed production database (create table/index
-- "if not exists", drop-then-create for policies/triggers, an existence
-- check for constraints — some have dependent foreign keys and cannot be
-- safely dropped and re-added), so `supabase db push` against the live
-- project is a no-op for this file.
--
-- Migrations squashed into this baseline (superseded, removed from the
-- repo, still present in the deployed project's migration history):
--   20260808125414_initial_invoice_app_schema.sql
--   20260808125802_harden_rls_helper_functions.sql
--   20260808144309_email_mfa_challenges.sql
--   20260808183548_add_invoice_vat_amounts.sql
--   20260808194513_fix_report_label_encoding.sql
--   20260819203652_fix_member_role_audit_variable.sql
--   20260821094121_delete_member_auth_account.sql
--   20260903175926_harden_auth_and_database_operations.sql
--   20260908110306_reminder_policy_categories.sql
--   20260908112046_counterparty_reminder_preferences.sql
--   20260908112533_index_counterparty_reminder_preferences.sql
--   20260908160937_optimize_reminder_queue.sql
--   20260908165426_add_organization_ocr_limit.sql
--   20260908174052_store_invoice_ocr_field_sources.sql
--
...
create table if not exists organization_member_events (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references organizations(id) on delete cascade,
...
create index if not exists organization_member_events_org_created on organization_member_events (organization_id, created_at desc, id desc);
...
create unique index if not exists reminder_policies_one_default on reminder_policies (organization_id) where is_default;

create index if not exists reminder_policies_active_org_idx on reminder_policies (organization_id, is_default, name) where archived_at is null;

create unique index if not exists reminder_policies_active_name_unique on reminder_policies (organization_id, lower(trim(name))) where archived_at is null;
...
create unique index if not exists invoices_one_document on invoices (file_url) where file_url is not null;
...
create index if not exists invoice_uploads_cleanup on invoice_uploads (status, expires_at);
...
create index if not exists reminder_automation_runs_org_started on reminder_automation_runs (organization_id, started_at desc);

create unique index if not exists reminder_automation_runs_one_running_per_org on reminder_automation_runs (organization_id)
  where status = 'running';

-- Nemenný audit každé změny plánu a textů automatických upomínek.
create table if not exists reminder_settings_events (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references organizations(id) on delete cascade,
  actor_user_id uuid references auth.users(id) on delete set null,
  actor_email text not null check (actor_email = lower(actor_email)),
...
create index if not exists reminder_settings_events_org_created on reminder_settings_events (organization_id, created_at desc, id desc);
...
create index if not exists invoices_org_status_due on invoices (organization_id, status, due_date);
...
create index if not exists reminder_log_invoice on reminder_log (invoice_id, created_at desc);

create index if not exists reminder_log_queue on reminder_log (status, scheduled_for) where status = 'queued';
...
do $guard$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'reminder_policies_org_id_key'
      and conrelid = 'reminder_policies'::regclass
  ) then
...
do $guard$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'invoices_org_id_key'
...
do $guard$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'invoices_policy_same_org_fkey'
      and conrelid = 'invoices'::regclass
  ) then
    alter table invoices
...
do $guard$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'invoice_events_invoice_same_org_fkey'
      and conrelid = 'invoice_events'::regclass
  ) then
...
do $guard$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'invoice_uploads_invoice_same_org_fkey'
      and conrelid = 'invoice_uploads'::regclass
  ) then
...
do $guard$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'reminder_log_invoice_same_org_fkey'
      and conrelid = 'reminder_log'::regclass
  ) then
...
do $guard$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'bank_payments_invoice_same_org_fkey'
      and conrelid = 'bank_payments'::regclass
  ) then
...
do $guard$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'invoices_paid_state_check'
      and conrelid = 'invoices'::regclass
  ) then
    alter table invoices
...
do $guard$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'invoices_payment_balance_state_check'
      and conrelid = 'invoices'::regclass
  ) then
    alter table invoices
...
do $guard$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'invoices_closed_schedule_check'
      and conrelid = 'invoices'::regclass
  ) then
    alter table invoices
...
do $guard$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'reminder_log_sent_state_check'
      and conrelid = 'reminder_log'::regclass
  ) then
    alter table reminder_log
...
do $guard$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'reminder_log_provider_state_check'
      and conrelid = 'reminder_log'::regclass
  ) then
    alter table reminder_log
...
drop policy if exists "members can view organization member events" on organization_member_events;

create policy "members can view organization member events" on organization_member_events for select to authenticated
  using (private.is_org_member(organization_id));

drop policy if exists "members can view policies" on reminder_policies;

create policy "members can view policies" on reminder_policies for select to authenticated
  using (private.is_org_member(organization_id));

drop policy if exists "members can view templates" on email_templates;
...
drop policy if exists "members can view invoice uploads" on invoice_uploads;

create policy "members can view invoice uploads" on invoice_uploads for select to authenticated
  using (private.is_org_member(organization_id));

drop policy if exists "accounting can create invoices" on invoices;
...
drop policy if exists "accounting can update invoices" on invoices;
...
drop policy if exists "members can view reminder log" on reminder_log;
...
drop policy if exists "members can view reminder settings events" on reminder_settings_events;
...
create index if not exists email_mfa_challenges_user_session_created on email_mfa_challenges (user_id, session_id, created_at desc);
...
create table if not exists counterparty_reminder_preferences (
  organization_id uuid not null references organizations(id) on delete cascade,
...
create index if not exists counterparty_reminder_preferences_policy_idx on counterparty_reminder_preferences (organization_id, reminder_policy_id);

create index if not exists counterparty_reminder_preferences_invoice_idx on counterparty_reminder_preferences (organization_id, last_invoice_id);

create index if not exists counterparty_reminder_preferences_updated_by_idx on counterparty_reminder_preferences (updated_by);
...
drop trigger if exists remember_ocr_invoice_reminder_policy on invoices;
...
create index if not exists bank_payments_imported_by_idx on bank_payments (imported_by);

create index if not exists bank_payments_org_invoice_idx on bank_payments (organization_id, invoice_id);

create index if not exists bank_payments_unmatched_by_idx on bank_payments (unmatched_by);

create index if not exists invoice_events_actor_user_id_idx on invoice_events (actor_user_id);

create index if not exists invoice_events_org_invoice_idx on invoice_events (organization_id, invoice_id);

create index if not exists invoice_uploads_created_by_idx on invoice_uploads (created_by);

create index if not exists invoice_uploads_invoice_id_idx on invoice_uploads (invoice_id);

create index if not exists invoice_uploads_org_invoice_idx on invoice_uploads (organization_id, invoice_id);

create index if not exists invoices_created_by_idx on invoices (created_by);

create index if not exists invoices_org_policy_idx on invoices (organization_id, reminder_policy_id);

create index if not exists invoices_reminder_policy_id_idx on invoices (reminder_policy_id);

create index if not exists invoices_reminders_paused_by_idx on invoices (reminders_paused_by);

create index if not exists invoices_updated_by_idx on invoices (updated_by);

create index if not exists organization_member_events_actor_idx on organization_member_events (actor_user_id);

create index if not exists organization_members_user_id_idx on organization_members (user_id);

create index if not exists reminder_automation_runs_triggered_by_idx on reminder_automation_runs (triggered_by);

create index if not exists reminder_log_org_invoice_idx on reminder_log (organization_id, invoice_id);

create index if not exists reminder_settings_events_actor_idx on reminder_settings_events (actor_user_id);
...
create index if not exists auth_request_events_window_idx on auth_request_events (action, subject_hash, requested_at desc);

create index if not exists auth_request_events_retention_idx on auth_request_events (requested_at);
