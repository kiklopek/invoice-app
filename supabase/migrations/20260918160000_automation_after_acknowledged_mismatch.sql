-- Automation must survive an acknowledged account mismatch.
--
-- A statement whose header account does not match the organization's configured
-- one is held back for a human to confirm -- correctly, since booking money from
-- the wrong account is unrecoverable. But the hold was permanent: confirming the
-- mismatch recorded account_mismatch_acknowledged and nothing ever read it back.
-- The worker's filter tested the raw account_mismatch flag and the reconciler
-- re-raised on every unattended pass, so an import the user had already cleared
-- stayed excluded from automatic booking forever.
--
-- Both now read the acknowledgement. An unacknowledged mismatch still blocks the
-- unattended path exactly as before -- what changes is only that "the user said
-- yes to this statement" is remembered.
...
-- The worker's own filter has to read the acknowledgement too, otherwise the
-- relaxation above is unreachable: the import would never be selected at all.
