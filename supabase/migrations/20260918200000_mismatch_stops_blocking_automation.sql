-- An account mismatch stops blocking unattended booking.
--
-- The check compares the account in the statement header against the one
-- configured for the organization. For this owner's bank those never agree --
-- the header carries a value that is not the account number at all -- so the
-- gate fired on every single import and held the whole statement, including the
-- rows matched on an exact variable symbol.
--
-- At the owner's explicit decision it now only gates a human commit, where a
-- person is looking at the warning and can answer it. The detection, the flag
-- and the warning in the UI are all unchanged; what is gone is the gate on the
-- unattended path. The risk this accepts is narrow but real: a statement
-- uploaded from the wrong account will be processed without anyone being asked.
...
-- The worker's filter has to drop the mismatch condition too, otherwise the
-- import is never selected and the change above is unreachable.
