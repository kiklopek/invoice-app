...
import { canAccessPage, landingPageForRole, roleNames, type AccessRole } from "@/lib/role-access";
...
type NavItem = { href: string; label: string; icon: IconName; children?: NavChild[] };
...
const items: NavItem[] = [
  { href: "/dashboard", label: "Přehled", icon: "dashboard" },
  {
    ...
    children: [
      { href: "/invoices/import", label: "Přidat fakturu" },
      { href: "/invoices/archive", label: "Archiv" },
    ],
  },
  {
    href: "/invoices/payments", label: "Platby", icon: "bank",
    children: [
...
function visibleNavForRole(role: AccessRole | null): NavItem[] {
  ...
  return items
    ...
    .map(item => ({ ...item, children: item.children?.filter(child => canAccessPage(role, child.href)) }));
}
...
export function AppSidebar({ invoiceCount, initialProfile }: { invoiceCount?: number; initialProfile: AccessProfile | null }) {
  ...
  const isActive = (href: string) => {
    ...
    return !visibleItems.some(other => other.href !== href && other.href.startsWith(`${href}/`)
  ...
  return (
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav desktop-navigation" data-role={role} aria-label="Hlavní navigace">
        <span className="nav-heading">Hlavní nabídka</span>
        {visibleItems.map(item => {
          const active = isActive(item.href);
          return <div key={item.href} className="nav-item-group">
            <Link href={item.href} prefetch={true} aria-current={active ? "page" : undefined} className={[active ? "active" : "", item.href === "/dashboard" ? "nav-primary" : ""].filter(Boolean).join(" ")}><span className="nav-symbol"><Icon name={item.icon}/></span><span>{item.label}</span>{item.href === "/invoices" && invoiceCount ? <em>{invoiceCount}</em> : null}</Link>
            {active && item.children && item.children.length > 0 ? (
              <div className="nav-subitems">
                {item.children.map(child => (
                  <Link key={child.href} href={child.href} prefetch={true} aria-current={pathname === child.href ? "page" : undefined} className={pathname === child.href ? "active" : ""}>{child.label}</Link>
                ))}
              </div>
            ) : null}
          </div>;
      ...
      <div className="mobile-navigation-shell">
        <div className="mobile-navigation-bar">
          <Link href={landingPageForRole(role)} className="mobile-navigation-brand" aria-label="Přejít na přehled">
            <CompanyLogo className="mobile-navigation-logo" />
          </Link>
          <div className="mobile-navigation-current">
            {activeItem && <span><Icon name={activeItem.icon}/></span>}
            <div><small>Aktuální sekce</small><strong>{activeItem?.label ?? "Navigace"}</strong></div>
          </div>
          <button
            type="button"
        ...
        {mobileNavOpen && <>
          <button type="button" className="mobile-navigation-backdrop" aria-label="Zavřít navigaci" onClick={() => setMobileNavOpen(false)}/>
          <div id="mobile-navigation-panel" className="mobile-navigation-panel">
            <div className="mobile-navigation-panel-heading"><span>Navigace</span><small>Vyberte část aplikace</small></div>
            <nav className="mobile-navigation-links" aria-label="Mobilní navigace">
              {visibleItems.map(item => {
                const active = isActive(item.href);
                return <Fragment key={item.href}>
                  <Link href={item.href} prefetch={true} aria-current={active ? "page" : undefined} className={active ? "active" : ""} onClick={() => setMobileNavOpen(false)}><span className="nav-symbol"><Icon name={item.icon}/></span><span>{item.label}</span>{item.href === "/invoices" && invoiceCount ? <em>{invoiceCount}</em> : null}</Link>
                  {active && item.children && item.children.length > 0 ? (
                    <div className="mobile-navigation-subitems">
                      {item.children.map(child => (
                        <Link key={child.href} href={child.href} prefetch={true} aria-current={pathname === child.href ? "page" : undefined} className={pathname === child.href ? "active" : ""} onClick={() => setMobileNavOpen(false)}>{child.label}</Link>
                      ))}
                    </div>
                  ) : null}
                </Fragment>;
            ...
            {role && <div className="mobile-navigation-profile">
              <div className="avatar">{profile ? profileInitials(profile.name, profile.email) : "…"}</div>
              <div><strong>{profile?.name ?? "Uživatel"}</strong><small>{profile?.companyName ?? ""}{profile && role ? ` · ${roleNames[role]}` : ""}</small></div>
              <button type="button" onClick={signOut} disabled={signingOut} aria-label={signingOut ? "Odhlašuji" : "Odhlásit se"}><Icon name="logout"/></button>
            </div>}
            {logoutError && <p className="mobile-navigation-error" role="alert">{logoutError}</p>}
          </div>
        </>}
      </div>
