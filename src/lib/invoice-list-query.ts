...
const statuses: Array<InvoiceStatus | "closed"> = ["pending", "overdue", "paid", "cancelled", "closed"];
...
export function parseInvoiceListQuery(params: URLSearchParams): InvoiceListQuery | null {
  ...
  if (rawStatus && !statuses.includes(rawStatus as InvoiceStatus | "closed")) return null;
  ...
  return { query, status: rawStatus as InvoiceStatus | "closed" | null, currency: rawCurrency, from, to, page };
}
