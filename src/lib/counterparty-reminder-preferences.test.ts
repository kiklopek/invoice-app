...
import { normalizeCounterpartyIco, resolveReminderPolicyPreference } from "./counterparty-reminder-preferences";
...
describe("counterparty reminder preferences", () => {
  ...
  it("uses a remembered active policy for a known ICO", () => {
    expect(resolveReminderPolicyPreference({ counterpartyIco: "12345678", preferredPolicyId: "vip", policies }))
  ...
  it("asks for a check while preselecting the default for a new ICO", () => {
    expect(resolveReminderPolicyPreference({ counterpartyIco: "87654321", preferredPolicyId: null, policies }))
  ...
  it("falls back safely when a remembered policy is archived", () => {
    expect(resolveReminderPolicyPreference({ counterpartyIco: "12345678", preferredPolicyId: "archived", policies }))
  ...
  it("reports a missing ICO while keeping invoice creation usable", () => {
    expect(resolveReminderPolicyPreference({ counterpartyIco: "", preferredPolicyId: null, policies }))
