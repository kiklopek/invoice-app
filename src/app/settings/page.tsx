...
export default function SettingsPage() {
  ...
  async function removeMember(member: Member) { if (!window.confirm(`Opravdu odebrat přístup pro ${member.email}? Přihlašovací účet bude smazán. Při případném opětovném přidání musí uživatel projít novou registrací a ověřit e-mail.`)) return; setSaving(true); setMessage(""); try { const response = await fetch("/api/settings/members", { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: member.id }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); await refreshMembers(); setMessage("Přístup i přihlašovací účet byly smazány. Po opětovném přidání si uživatel vytvoří nový účet."); } catch (cause) { await refreshMembers().catch(() => undefined); setMessage(cause instanceof Error ? cause.message : "Přístup se nepodařilo odebrat."); } finally { setSaving(false); } }
