...
export default function SettingsPage() {
  ...
  async function addMember(event: React.FormEvent) { event.preventDefault(); setSaving(true); setMessage(""); try { const response = await fetch("/api/settings/members", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: newEmail, role: newRole }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); await refreshMembers(); setNewEmail(""); setMessage("Přístup je přidaný. Uživatel si nyní může vytvořit účet a přihlásit se heslem."); } catch (cause) { setMessage(cause instanceof Error ? cause.message : "Přístup se nepodařilo přidat."); } finally { setSaving(false); } }
  ...
  async function removeMember(member: Member) { if (!window.confirm(`Opravdu odebrat přístup pro ${member.email}? Uživatel se už do aplikace nedostane.`)) return; setSaving(true); setMessage(""); try { const response = await fetch("/api/settings/members", { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: member.id }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); await refreshMembers(); setMessage("Přístup byl odebrán. Uživatel se už do firemní aplikace nedostane."); } catch (cause) { await refreshMembers().catch(() => undefined); setMessage(cause instanceof Error ? cause.message : "Přístup se nepodařilo odebrat."); } finally { setSaving(false); } }
