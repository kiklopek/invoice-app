...
export async function GET() {
  ...
  if (identity.membership.role === "viewer") return NextResponse.json({ error: "Čtenář nemá přístup k nastavení firmy." }, { status: 403 });
...
export async function PUT(request: Request) {
  ...
  const identity = await getRequestIdentity();
  if (!identity) return NextResponse.json({ error: "Nejste přihlášený uživatel." }, { status: 401 });
  if (identity.membership.role !== "admin") return NextResponse.json({ error: "Firemní údaje může měnit pouze administrátor." }, { status: 403 });
