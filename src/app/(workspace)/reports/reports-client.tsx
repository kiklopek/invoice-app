          ...
          <div className="report-tab-panels">
            ...
            <section id="report-panel-vat" role="tabpanel" aria-labelledby="report-tab-vat" aria-hidden={activeTab !== "vat"} className={`report-tab-panel${activeTab === "vat" ? " active" : ""}`}>
              ...
              <div className="report-workspace-grid report-vat-grid">
                <article className="page-panel report-card report-span-8"><header><div><h3>Rozpis podle sazby DPH</h3><p>Podklad pro kontrolu daňového přiznání</p></div></header><div className="report-accounting-table"><table className="vat-breakdown-table"><thead><tr><th>Sazba</th><th>Základ daně</th><th>DPH</th><th>Celkem</th><th>Faktur</th></tr></thead><tbody>{report.vat_breakdown.map((row) => <tr key={row.vat_rate}><td><strong>{row.vat_rate} %</strong></td><td>{money(Number(row.base), currency)}</td><td>{money(Number(row.tax), currency)}</td><td>{money(Number(row.gross), currency)}</td><td>{row.count}</td></tr>)}</tbody><tfoot><tr><td><strong>Celkem</strong></td><td><strong>{money(netTotal, currency)}</strong></td><td><strong>{money(taxTotal, currency)}</strong></td><td><strong>{money(Number(report.total), currency)}</strong></td><td><strong>{report.vat_breakdown.reduce((sum, row) => sum + row.count, 0)}</strong></td></tr></tfoot></table></div></article>
            ...
            <section id="report-panel-receivables" role="tabpanel" aria-labelledby="report-tab-receivables" aria-hidden={activeTab !== "receivables"} className={`report-tab-panel${activeTab === "receivables" ? " active" : ""}`}>
              ...
              <div className="report-workspace-grid">
                ...
                <article className="page-panel report-card report-span-12"><header><div><h3>Přehled dlužníků</h3><p>Kliknutím na řádek omezíte celý report na vybraného odběratele</p></div></header><div className="report-accounting-table"><table className="top-debtors-table"><thead><tr><th>Odběratel</th><th>Otevřeno</th><th>Po splatnosti</th><th>Faktur</th><th>Upomínky</th></tr></thead><tbody>{report.debtors.map((row) => <tr key={row.name} role="button" tabIndex={0} onClick={() => setCustomer(row.name)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setCustomer(row.name); } }}><td><strong>{row.name}</strong></td><td>{money(Number(row.open), currency)}</td><td className={row.overdue ? "red-text" : undefined}>{money(Number(row.overdue), currency)}</td><td>{row.count}</td><td>{row.reminders}</td></tr>)}</tbody></table></div></article>
              </div>
            </section>
