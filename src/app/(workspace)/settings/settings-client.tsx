...
import type { CompanySettings, SettingsAccessEvent, SettingsMember, SettingsPageData } from "@/lib/settings-page-data";
...
export function SettingsClient({ initialData }: { initialData: SettingsPageData }) {
  ...
  const [accessEvents, setAccessEvents] = useState<AccessEvent[]>(initialData.access_events);
  ...
  const { data: refreshedData, error: loadError, mutate: refreshSettings } = useSWR<SettingsPageData>("/api/settings/page-data", { fallbackData: initialData, revalidateOnMount: false });
