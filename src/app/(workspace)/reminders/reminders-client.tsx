...
import type { ReminderPageData, ReminderTemplateSettings, SettingsChange, ReminderOperations } from "@/lib/reminder-page-data";
...
export function RemindersClient({ initialData }: { initialData: ReminderPageData }) {
  const initialPolicy = initialData.policies.find(policy => policy.is_default && !policy.archived_at) ?? initialData.policies.find(policy => !policy.archived_at);
  const [rules, setRules] = useState<Rule[]>(() => toRules(initialPolicy?.days_from_due ?? initialData.settings.days));
  const [policies, setPolicies] = useState<ReminderPolicySummary[]>(initialData.policies);
  const [selectedPolicyId, setSelectedPolicyId] = useState(initialPolicy?.id ?? "");
  const [policyName, setPolicyName] = useState(initialPolicy?.name ?? "Standardní");
  const [creatingPolicy, setCreatingPolicy] = useState(false);
  const [policyDirty, setPolicyDirty] = useState(false);
  const [templates, setTemplates] = useState<Record<ReminderStage, Template> | null>(initialData.settings.templates);
  const [ccInputs, setCcInputs] = useState<Record<ReminderStage, string>>(() => ccInputsFromTemplates(initialData.settings.templates));
  const [activeStage, setActiveStage] = useState<ReminderStage>("before_due");
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState("");
  const [operations, setOperations] = useState<ReminderOperations>(initialData.operations);
  const [automationActive, setAutomationActive] = useState(initialData.settings.active);
  const [lastChange, setLastChange] = useState<SettingsChange | null>(initialData.settings.last_change);
  const [company, setCompany] = useState<ReminderEmailCompany | null>(initialData.company);
  ...
  const automationQueueDetail = operations.automation_run && operations.automation_run.status !== "running"
  ...
  const automationRunDetail = !operations.automation_run ? "zatím bez zaznamenaného běhu"
    ...
    : operations.automation_run.status === "partial" ? `${automationRunOrigin} · ${operations.automation_run.failed} chyb · ${operations.automation_run.sent} odesláno${automationQueueDetail}`
  ...
  useUnsavedChanges(policyDirty);

  const { data: refreshedData, error: loadError, mutate: refreshPage } = useSWR<ReminderPageData>("/api/reminders/page-data", { fallbackData: initialData, revalidateOnMount: false });
  useEffect(() => { if (loadError instanceof Error) setMessage(loadError.message); }, [loadError]);
  useEffect(() => {
    if (!refreshedData || policyDirty || creatingPolicy) return;
    const refreshedPolicy = refreshedData.policies.find(policy => policy.id === selectedPolicyId)
      ?? refreshedData.policies.find(policy => policy.is_default && !policy.archived_at)
    ...
    setLoading(false);
  }, [creatingPolicy, policyDirty, refreshedData, selectedPolicyId]);
  ...
  function update(id: string, patch: Partial<Rule>) { setPolicyDirty(true); setRules(current => current.map(rule => rule.id === id ? { ...rule, ...patch } : rule)); }
  function add() { setPolicyDirty(true); setRules(current => current.length >= 10 ? current : [...current, { id: crypto.randomUUID(), relation: "after", days: 7 }]); }
  async function selectPolicy(id: string) { if (id === selectedPolicyId) return; if (policyDirty && !await confirmAction({ title: "Zahodit neuložené změny?", description: "Úpravy této kategorie ještě nejsou uložené.", confirmLabel: "Zahodit změny" })) return; const policy = policies.find(item => item.id === id); if (!policy) return; setCreatingPolicy(false); setSelectedPolicyId(id); setPolicyName(policy.name); setRules(toRules(policy.days_from_due)); setPolicyDirty(false); setMessage(""); }
  function startCreatingPolicy() { const source = policies.find(policy => policy.is_default && !policy.archived_at) ?? selectedPolicy; setCreatingPolicy(true); setSelectedPolicyId(""); setPolicyName(""); setRules(toRules(source?.days_from_due ?? [-3, 0, 7, 14])); setPolicyDirty(true); setMessage(""); }
  function categoryDays() { return rules.map(rule => rule.relation === "before" ? -Math.abs(rule.days) : rule.relation === "after" ? Math.abs(rule.days) : 0); }
  function validateCategory() { if (!policyName.trim()) return "Zadejte název kategorie."; if (!rules.length) return "Přidejte alespoň jednu upomínku."; if (rules.some(rule => !Number.isInteger(rule.days) || (rule.relation !== "on" && (rule.days < 1 || rule.days > (rule.relation === "before" ? 90 : 365))))) return "Zkontrolujte počet dní: před splatností nejvýše 90, po splatnosti nejvýše 365."; if (new Set(categoryDays()).size !== rules.length) return "Každý termín může být v kategorii jen jednou."; return null; }
  async function saveCategory(makeDefault = false) { const validation = validateCategory(); if (validation) { setMessage(validation); return; } setSaving(true); setMessage(""); try { const response = await fetch("/api/settings/reminder-policies", { method: creatingPolicy ? "POST" : "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...(!creatingPolicy && { id: selectedPolicyId }), name: policyName, days: categoryDays(), make_default: makeDefault || selectedPolicy?.is_default }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); const saved = data.policy as ReminderPolicySummary; setPolicies(current => creatingPolicy ? [...current, saved] : current.map(policy => saved.is_default ? { ...policy, is_default: policy.id === saved.id, ...(policy.id === saved.id ? saved : {}) } : policy.id === saved.id ? saved : policy)); setSelectedPolicyId(saved.id); setPolicyName(saved.name); setRules(toRules(saved.days_from_due)); setCreatingPolicy(false); setPolicyDirty(false); setMessage("Kategorie upomínek je uložená."); void refreshPage(); } catch (cause) { setMessage(cause instanceof Error ? cause.message : "Kategorii se nepodařilo uložit."); } finally { setSaving(false); } }
  async function deleteCategory() { if (!selectedPolicy || selectedPolicy.is_default || !await confirmAction({ title: "Smazat kategorii?", description: "Kategorie zmizí z nových voleb. Uložené faktury si kvůli historii ponechají její název a plán.", confirmLabel: "Smazat kategorii" })) return; setSaving(true); try { const response = await fetch(`/api/settings/reminder-policies?id=${encodeURIComponent(selectedPolicy.id)}`, { method: "DELETE" }); const data = await response.json(); if (!response.ok) throw new Error(data.error); const remaining = policies.filter(policy => policy.id !== selectedPolicy.id); const next = remaining.find(policy => policy.is_default) ?? remaining[0]; setPolicies(remaining); setSelectedPolicyId(next?.id ?? ""); setPolicyName(next?.name ?? ""); setRules(toRules(next?.days_from_due ?? [-3, 0, 7, 14])); setPolicyDirty(false); setMessage("Kategorie byla smazána."); void refreshPage(); } catch (cause) { setMessage(cause instanceof Error ? cause.message : "Kategorii se nepodařilo smazat."); } finally { setSaving(false); } }
  async function save() { if (!templates) return; setSaving(true); setMessage(""); const defaultDays = policies.find(policy => policy.is_default)?.days_from_due ?? [-3, 0, 7, 14]; const templatesToSave = Object.fromEntries(reminderStages.map(stage => [stage, { ...templates[stage], cc: parseReminderCcInput(ccInputs[stage]) }])); try { const response = await fetch("/api/settings/reminders", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ active: automationActive, days: defaultDays, templates: templatesToSave }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); setAutomationActive(data.active); setTemplates(data.templates); setCcInputs(ccInputsFromTemplates(data.templates)); setLastChange(data.last_change ?? lastChange); if (!data.active) setOperations(current => ({ ...current, upcoming: [] })); else { const overviewResponse = await fetch("/api/reminders"); const overview = await overviewResponse.json(); if (overviewResponse.ok) setOperations(overview); } setMessage(data.active ? "Společné nastavení a šablony jsou uložené." : "Automatické odesílání je bezpečně pozastavené."); } catch (cause) { setMessage(cause instanceof Error ? cause.message : "Nastavení se nepodařilo uložit."); } finally { setSaving(false); } }
  ...
  return <AppFrame><header className="section-header"><div><p>AUTOMATIZACE</p><h1>Upomínky</h1><span>Nastavte srozumitelný postup, podle kterého aplikace kontaktuje odběratele.</span></div>{operations.can_run && <div className="section-actions"><button className="btn secondary" disabled={runningNow || loading || saving} onClick={runNow}>{runningNow ? "Kontroluji…" : <><Icon name="clock"/>Spustit kontrolu</>}</button></div>}</header>
    ...
    <section className="reminder-operations reminder-quick-stats"><article className="page-panel"><span>Naplánováno</span><strong>{operations.upcoming.length}</strong><small>nejbližších automatických akcí</small></article><article className={`page-panel ${operations.failed.length || automationRunProblem ? "has-failures" : ""}`}><span>Vyžaduje kontrolu</span><strong>{operations.failed.length}</strong><small>{automationRunProblem && !operations.failed.length ? "poslední běh automatu vyžaduje kontrolu" : "problémů s odesláním nebo doručením"}</small></article></section>
    <section className="page-panel policy-toolbar"><div><h2 id="reminder-policy-heading">Kategorie upomínek</h2><div className="policy-select-wrap"><select id="reminder-policy-select" aria-labelledby="reminder-policy-heading" value={selectedPolicyId} disabled={loading || creatingPolicy} onChange={event => void selectPolicy(event.target.value)}>{policies.filter(policy => !policy.archived_at).map(policy => <option key={policy.id} value={policy.id}>{policy.name}{policy.is_default ? " · Výchozí" : ""}</option>)}</select></div></div><button type="button" className="btn primary" disabled={!operations.can_run || creatingPolicy || policyDirty} onClick={startCreatingPolicy}>+ Přidat kategorii</button></section>
    <div className="reminders-layout"><section className="page-panel rules-panel"><header><div><h2>{creatingPolicy ? "Nová kategorie" : "Kdy se mají upomínky posílat?"}</h2><p>Změny se použijí pro nové faktury. Uložené faktury si ponechají svůj plán.</p></div><button className="btn secondary" disabled={!operations.can_run || rules.length >= 10} onClick={add}>+ Přidat další upomínku</button></header>{loading ? <p className="page-state">Načítám pravidla…</p> : <><div className="policy-name-row"><label><span>Název kategorie</span><input maxLength={100} value={policyName} disabled={!operations.can_run} placeholder="např. Klíčoví zákazníci" onChange={event => { setPolicyName(event.target.value); setPolicyDirty(true); }}/></label>{selectedPolicy?.is_default && <span className="policy-default-badge">Výchozí</span>}</div><div className="human-rules">{sorted.map((rule, index) => <article key={rule.id}><div className="rule-order">{index + 1}</div><div className="rule-main"><strong>{sentence(rule)}</strong><div className="rule-controls"><span>Odeslat</span>{rule.relation !== "on" && <input aria-label="Počet dní" type="number" min="1" max={rule.relation === "before" ? 90 : 365} value={rule.days} disabled={!operations.can_run} onChange={e => update(rule.id, { days: Number(e.target.value) })}/>}<select aria-label="Vztah ke splatnosti" value={rule.relation} disabled={!operations.can_run} onChange={e => update(rule.id, { relation: e.target.value as Rule["relation"], days: e.target.value === "on" ? 0 : rule.days || 1 })}><option value="before">dní před splatností</option><option value="on">v den splatnosti</option><option value="after">dní po splatnosti</option></select></div></div><button type="button" className="remove-rule" disabled={!operations.can_run || rules.length === 1} onClick={() => { setPolicyDirty(true); setRules(current => current.filter(item => item.id !== rule.id)); }} aria-label="Odstranit pravidlo">×</button></article>)}</div><div className="policy-actions">{creatingPolicy && <button type="button" className="btn secondary" onClick={() => { const fallback = policies.find(policy => policy.is_default) ?? policies[0]; if (fallback) void selectPolicy(fallback.id); }}>Zrušit</button>}{selectedPolicy && !selectedPolicy.is_default && <button type="button" className="btn secondary" disabled={saving || policyDirty} onClick={() => void saveCategory(true)}>Nastavit jako výchozí</button>}{selectedPolicy && !selectedPolicy.is_default && <button type="button" className="btn secondary danger-button" disabled={saving || policyDirty} onClick={() => void deleteCategory()}>Smazat kategorii</button>}<button type="button" className="btn primary" disabled={!operations.can_run || saving || !policyDirty} onClick={() => void saveCategory()}>{saving ? "Ukládám…" : "Uložit kategorii"}</button></div></>}</section>
    ...
    <details className={`page-panel reminder-section-disclosure ${operations.failed.length || automationRunProblem ? "has-alert" : ""}`}>
      <summary><span><strong>Provozní přehled a historie</strong><small>Poslední běh automatu, naplánované upomínky a položky ke kontrole.</small></span><span className="reminder-disclosure-meta">{operations.failed.length > 0 ? <em>{operations.failed.length} {operations.failed.length === 1 ? "problém" : operations.failed.length < 5 ? "problémy" : "problémů"}</em> : automationRunProblem ? <em>Vyžaduje kontrolu</em> : null}<i aria-hidden="true"/></span></summary>
      <div className="reminder-disclosure-body">
        <section className="reminder-operations reminder-detail-stats"><article><span>Naposledy odesláno</span><strong>{operations.recent.length ? formatDate(operations.recent[0].sent_at) : "—"}</strong><small>{operations.recent.length ? operations.recent[0].invoices.invoice_number : "zatím bez zpráv"}</small></article><article className={automationRunProblem ? "has-failures" : ""}><span>Poslední běh automatu</span><strong>{operations.automation_run ? formatDateTime(operations.automation_run.started_at) : "—"}</strong><small>{automationRunDetail}</small></article></section>
        <div className="reminder-monitor-grid"><section className="reminder-monitor"><header><div><h2>Nejbližší upomínky</h2><p>Co automat odešle podle aktuálního plánu.</p></div></header><div>{operations.upcoming.slice(0, 6).map(item => <Link href={`/invoices/${item.id}`} key={item.id} className="monitor-row"><span><strong>{item.invoice_number}</strong><small>{item.counterparty_name}</small></span><span><strong>{formatDate(item.next_reminder_at)}</strong><small>{formatMoney(item.amount, item.currency)}</small></span></Link>)}{!operations.upcoming.length && <p className="page-state">Žádné upomínky nejsou naplánované.</p>}</div></section><section className="reminder-monitor"><header><div><h2>Vyžaduje kontrolu</h2><p>Chyby odeslání i problémy s následným doručením.</p></div></header><div>{operations.failed.slice(0, 6).map(item => <Link href={`/invoices/${item.invoice_id}`} key={item.id} className="monitor-row failed"><span><strong>{item.invoices.invoice_number}</strong><small>{item.invoices.counterparty_name} · {item.sent_to}</small></span><span><strong>{item.delivery_status ? item.delivery_status === "delayed" ? "Odložené doručení" : "Nedoručeno" : `${item.attempt_count}. pokus`}</strong><small>{item.error_message || "E-mail se nepodařilo odeslat"}</small></span></Link>)}{!operations.failed.length && <p className="page-state success-state">Všechna odeslání i doručení jsou v pořádku.</p>}</div></section></div>
      </div>
    </details>
    {templates && <details className="page-panel reminder-section-disclosure reminder-email-settings">
      <summary><span><strong>Texty e-mailů pro všechny kategorie</strong><small>Společné texty, předměty a adresy používané u všech kategorií.</small></span><span className="reminder-disclosure-meta"><i aria-hidden="true"/></span></summary>
      <section className="templates-page reminder-disclosure-body">
      <div className="template-header-actions reminder-template-actions"><button type="button" className="btn secondary" disabled={!operations.can_run} onClick={useRecommendedTemplate}>Použít doporučený text</button><button type="button" className="btn secondary" disabled={sendingTest || !operations.can_run} onClick={sendTest}>{sendingTest ? "Odesílám test…" : "Poslat test na můj e-mail"}</button></div>
