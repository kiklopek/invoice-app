...
export function PaymentsClient({ initialData }: { initialData: PaymentsPageData }) {
  ...
  const [gpcEnabled, setGpcEnabled] = useState<boolean>(initialData.gpc_enabled);
  const [gpcDiagnostic, setGpcDiagnostic] = useState<string | null>(
    initialData.gpc_enabled ? null : "Import bankovních výpisů není pro vaši roli nebo toto prostředí povolený.",
  );
